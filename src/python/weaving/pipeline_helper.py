...
from gcmaker_helper import linkageData
...
JSON_PATH = osp.join(weaving_dir + 'normalized_objs/json/{}.json'.format(MODEL_NAME))
...
class InputOrganizer():
    def __init__(self, model_name, thickness, width, weaving_dir, cross_section_scale=None):
        ...
        self.JSON_PATH = osp.join(weaving_dir + 'grasshopper/inputs/{}.json'.format(model_name))
...
def initialize_linkage_json(input_organizer : InputOrganizer):
    l = linkageData(input_organizer.JSON_PATH)
    if l.isValid:
        input_organizer.SUBDIVISION_RESOLUTION = l.subdivision
        #input_organizer.RIBBON_CS = l.Ribbon_CS
        attracted_surface_linkage = l.surface_attracted_linkage
        attracted_surface_linkage.set_holdClosestPointsFixed(True)
        attracted_surface_linkage.set_attraction_tgt_joint_weight(0.01)
        attracted_surface_linkage.attraction_weight = 100
        return l
    else: 
        return None
...
def initialize_stage_2_optimizer_json(linkage, target_vertices, target_faces, tgt_joint_pos, linkage_view, rest_length_weight, smoothing_weight, fixed_boundary_joint = [], equilibrium_iter = 25, inner_gradTol = 1e-6):
    OPTS = py_newton_optimizer.NewtonOptimizerOptions()
    OPTS.gradTol = inner_gradTol
    OPTS.verbose = 1
    OPTS.beta = 1e-8
    ...
    useCenterline = True
    optimizer = linkage_optimization.WeavingOptimizationFromData(linkage, target_vertices, target_faces, useCenterline, OPTS, pinJoint = 0, useFixedJoint = False, fixedVars = fixed_boundary_joint)
    optimizer.set_target_joint_position(tgt_joint_pos)
    if linkage_view:
        linkage_view.update()
...
def stage_2_optimization(optimizer, linkage, input_surface_path, tgt_joint_pos, linkage_view, minRestLen, fixed_boundary_joints, update_attraction_weight = -5, num_updates = 2, return_weight_change_iteration = False, update_color = False, no_surface = False, callback_freq = 1, maxIter = 2000):
    ...
    # algorithm = linkage_optimization.OptAlgorithm.BFGS
    prev_vars = linkage.getExtendedDoFsPSRL()
    ...
    if num_updates == 1:
        ...
        for curr_update_weight in np.logspace(2, update_attraction_weight, num_updates):
            weight_change_iterations.append(cb.numIterations())
            print('stage 2 optimization with attraction weight {}'.format(curr_update_weight), flush=False)
            prev_time_stamp = time.time()
...
def set_design_parameters_from_topology(curved_linkage, io):
    ...
    def set_segment_rest_kappa(curved_linkage, i):
        ...
        def get_rest_kappa(e1, e2, pn):
            # print(la.norm(e1) * la.norm(e2) + np.dot(e1, e2))
