...
namespace ErodModel.Model
{
    public class OpenLinkageOptsGH : GH_Component
    {
        ...
        public OpenLinkageOptsGH()
          : base("SolverOptions", "SolverOptions",
        ...
        {
            pManager.AddIntegerParameter("Steps", "Steps", "Number of opening steps.", GH_ParamAccess.item);
            pManager.AddIntegerParameter("Iter", "Iterations", "Maximum number of newton iterations per opening step.", GH_ParamAccess.item);
            pManager.AddIntegerParameter("IterMult", "IterMultiplier", "Iteration multiplier for the last opening step.", GH_ParamAccess.item, 2);
            ...
            pManager.AddNumberParameter("TempSupp", "TempSupp", "Indicator, from 0 (undeploy) to 1(deploy), for removing temporary supports.", GH_ParamAccess.item, 0.5);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            bool verbose = true, useIdentityMetric = false, verboseNonPosDef = true,
                useNegativeCurvatureDirection = true, feasibilitySolve = true;
            double gradTol = 1e-8, beta = 1e-8;
            int steps = 20, iter = 20, iterMult =1;
            double tempSupportIndicator = 0.5;
            ...
            DA.GetData(8, ref tempSupportIndicator);
            ...
            opts.TemporarySupportIndicator = tempSupportIndicator;
            if (tempSupportIndicator > 1) opts.TemporarySupportIndicator = 1;
            if (tempSupportIndicator < 0) opts.TemporarySupportIndicator = 0;
