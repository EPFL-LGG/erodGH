...
{
    public class SplineDataGH : GH_Component
    {
        int edgeLabel;
        ...
        {
            pManager.AddCurveParameter("Curve", "Crv", "Curve or line defining the spline-beam.", GH_ParamAccess.item);
            pManager.AddNumberParameter("t", "t", "Parameters to split.", GH_ParamAccess.list);
            pManager.AddIntegerParameter("Subdivision", "Subdivision", "Number of edges per rod-segment. The minimum number is 5.", GH_ParamAccess.list, new List<int>() { 10 });
            pManager.AddNumberParameter("Length", "Length", "Set the rest length of the spline-beam. If no length is set, the current length is taken.", GH_ParamAccess.item);
        ...
        {
            pManager.AddGenericParameter("Edge-beams", "E", "Edge-beams data.", GH_ParamAccess.list);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            for(int i=0; i< segments.Length; i++)
            {
                ...
                if (l > 0.01)
                {
                    SegmentData edge = new SegmentData(p1, p2, c, c.GetLength(), subdivision, tol);
                    edge.EdgeBeamLocalIndex = idx;
