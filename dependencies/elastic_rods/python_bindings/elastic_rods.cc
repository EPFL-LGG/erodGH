...
#include "../weaver_editor/weaver_editor_set_up.hh" // Doesn't actually need knitro; really just configures a target-attracted linkage
...
PYBIND11_MODULE(elastic_rods, m) {
    ...
    m.def("setUpLinkageForOptimization_surface_attracted_linkage", 
        [](SurfaceAttractedLinkage &linkage,  double linkage_attraction_weight, bool holdClosestPointsFixed, std::string cross_section_path, bool useFixedJoint) {
            py::scoped_ostream_redirect stream1(std::cout, py::module::import("sys").attr("stdout"));
            py::scoped_ostream_redirect stream2(std::cerr, py::module::import("sys").attr("stderr"));
            return setUpLinkageForOptimization(linkage, linkage_attraction_weight, holdClosestPointsFixed, cross_section_path, useFixedJoint);
        },
        py::arg("linkage"),
        py::arg("linkage_attraction_weight"),
        py::arg("holdClosestPointsFixed"),
        py::arg("cross_section_path"),
        py::arg("useFixedJoint")
    );
