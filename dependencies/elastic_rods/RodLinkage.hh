...
struct RodLinkage_T {
    ...
    void set(std::vector<MeshIO::IOVertex > vertices, 
                              std::vector<Vec3> vertices_normals,
...
// TODO (Samara): Need to refactor the rest length solve code.
    ...
    struct Joint {
        ...
        Joint(RodLinkage_T *l, const Pt3 &p, const Vec3 &n, const Vec3 &eA, const Vec3 &eB,
              const std::array<size_t, 2> segmentsA, const std::array<size_t, 2> segmentsB,
              const std::array<bool  , 2>  isStartA, const std::array<bool  , 2>  isStartB);

        Joint(RodLinkage_T *l, const Pt3 &p, const Vec3 &n, const Vec3 &eA, const Vec3 &eB,
              const std::array<size_t, 2> segmentsA, const std::array<size_t, 2> segmentsB,
              const std::array<bool  , 2>  isStartA, const std::array<bool  , 2>  isStartB, const Vec3 &t, Real_ signB, Real_ alpha);
        ...
        bool isNormalConsistent(const Vec3 &normal) const {
            return m_source_normal.dot(normal) >= 0;
        }
        ...
        // Flip orientation to best align with a given normal.
        // This is done by swapping the A and B labels (we want to preserve alpha's sign)
        void makeNormalConsistent(const Vec3 &normal) {
            if (!isNormalConsistent(normal)) swapRodLabels();
        }
        ...
        Real_       sign_B() const { return m_sign_B; }
        ...
        const Vec3 &source_tangent() const { return m_source_t; }
        ...
        const std::array<int, 4> &normal_signs() const { return m_normal_signs; }
