...
/////////////////////////////////////////////////////////////////////////
// Constructors for Grasshopper data
/////////////////////////////////////////////////////////////////////////
template<typename Real_>
void RodLinkage_T<Real_>::set(std::vector<MeshIO::IOVertex > vertices, 
                              std::vector<Vec3> input_joint_normals,
                              std::vector<MeshIO::IOElement> edges,
                              std::vector<std::vector<Pt3>> rodPoints,
                              VecX_T<Real_> restLengths,
                              bool initConsistentAngle,
                              bool initConsistentNormals,
                              InterleavingType rod_interleaving_type) {
    {
        const size_t old_nv = vertices.size();
        merge_duplicate_vertices(vertices, edges, vertices, edges, 1e-6);
    ...
    std::vector<RodSegment> temp_segments;
    std::vector<Joint> temp_joints;
    std::vector<size_t> temp_subdivisions(ne);
    temp_segments.clear();
    temp_joints.clear();
    temp_segments.reserve(edges.size());
    temp_joints.reserve(vertices.size());
    size_t minSubdivision = rodPoints[0].size()-1;
    ...
    for (int i=0; i<ne; ++i) {
        ...
        const size_t subdivision = rodPoints.at(i).size()-1;

        temp_segments.emplace_back(vertices[e[0]].point,
                                vertices[e[1]].point,
                                subdivision);
        temp_subdivisions[i] = subdivision;
        segmentRestLenGuess[i] = restLengths[i];

        if(subdivision < minSubdivision) minSubdivision = subdivision;
    }
    ...
    for (size_t vi = 0; vi < nv; ++vi) {
        ...
        // will need to be recomputed.
        Vec3 edgeA = -edgeVecs.col(segmentsA[0]), edgeB = -edgeVecs.col(segmentsB[0]); // get vector pointing out of segment 0
        const size_t subdivisionA = temp_subdivisions.at(incidentEdges[vi][segmentsA[0]]);
        const size_t subdivisionB = temp_subdivisions.at(incidentEdges[vi][segmentsB[0]]);
        Real_ segmentFracLenA = 1.0 / (subdivisionA - 1); // only (subdivision - 1) segment lengths fit between the endpoints; rod extends half a segment past each endpoint.
        Real_ segmentFracLenB = 1.0 / (subdivisionB - 1); // only (subdivision - 1) segment lengths fit between the endpoints; rod extends half a segment past each endpoint.
        Real_ lenA  = edgeVecLens[segmentsA[0]] * segmentFracLenA,
              lenB  = edgeVecLens[segmentsB[0]] * segmentFracLenB;
        if (numA == 2) {
            lenA  = std::min<Real_>(lenA, edgeVecLens[segmentsA[1]] * segmentFracLenA);
            edgeA += edgeVecs.col(segmentsA[1]);
        }
        if (numB == 2) {
            lenB  = std::min<Real_>(lenB, edgeVecLens[segmentsB[1]] * segmentFracLenB);
            edgeB += edgeVecs.col(segmentsB[1]);
        }
        edgeA *= lenA / edgeA.norm();
        edgeB *= lenB / edgeB.norm();
    ...
    for (size_t vi = 0; vi < nv; ++vi) {
        ...
        std::array<bool,  4> isStartPt;
        for (size_t k = 0; k < jointValence; ++k) {
            const size_t eIdx = incidentEdges[vi][k];

            const auto &e = edges.at(eIdx);
            isStartPt[k] = (m_rod_orientation_indicator[eIdx]) ? (e[0] == vi) : (e[1] == vi);
            const auto pts = rodPoints.at(eIdx);
            const size_t lastIdx = pts.size() - 1;

            // Estimate the tangent direction at the joint and use that as edge vectors.
            edgeVecs[k] = isStartPt[k] ? pts[1] - pts[0] : pts[lastIdx-1] - pts[lastIdx];
            edgeVecs[k] *= segmentRestLenGuess[eIdx] / edgeVecs[k].norm();
            edgeVecLens[k] = edgeVecs[k].norm();
        }
    ...
    for (size_t si = 0; si < ne; ++si) {
        // m_segments.at(si).startJoint = temp_segments.at(si).startJoint;
        // m_segments.at(si).endJoint = temp_segments.at(si).endJoint;
        const size_t startJoint = temp_segments.at(si).startJoint;
        const size_t endJoint = temp_segments.at(si).endJoint;
        const size_t numPts = rodPoints.at(si).size();
        std::vector<Pt3> pts;

        // Use the fact that int(true) == 1 to determine edge orientation. 
        if(m_rod_orientation_indicator[si]==0) for(size_t pi = numPts - 1; pi >= 0; --pi) pts.emplace_back(rodPoints.at(si).at(pi));
        else for(size_t pi = 0; pi<numPts; ++pi) pts.emplace_back(rodPoints.at(si).at(pi));

        Rod rod(pts);
        m_segments.emplace_back(startJoint, endJoint, std::move(rod)); 
    }

    // Propagate consistent joint normals throughout the graph using BFS
    // (to prevent 180 degree twists); warn if the graph is disconnected.
    if(initConsistentNormals){
        std::queue<size_t> bfsQueue;
        std::vector<bool> visited(nv, false);
        visited[firstJointVtx] = true;
        bfsQueue.push(firstJointVtx);
...
////////////////////////////////////////////////////////////////////////////////
// Constructor added to intialize a joint from Grasshopper data (Enforce normal)
////////////////////////////////////////////////////////////////////////////////
template<typename Real_>
RodLinkage_T<Real_>::Joint::Joint(RodLinkage_T *l, const Pt3 &p, const Vec3 &n, const Vec3 &eA, const Vec3 &eB,
                         const std::array<size_t, 2> segmentsA, const std::array<size_t, 2> segmentsB,
                         const std::array<bool  , 2>  isStartA, const std::array<bool  , 2>  isStartB)
    : m_linkage(l), m_pos(p),
      m_segmentsA(segmentsA), m_segmentsB(segmentsB),
      m_isStartA(isStartA), m_isStartB(isStartB)
{
    ...
    m_source_normal = tA.cross(tB);
    //if(m_source_normal.dot(n)<0) m_source_normal *= -1;
