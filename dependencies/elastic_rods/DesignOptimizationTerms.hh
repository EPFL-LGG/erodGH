...
// WARNING: only makes sense for RodLinkage_T-derived classes for now...
template<template<typename> class Object_T>
struct ContactForceObjective : public DesignOptimizationObjectiveTerm<Object_T> {
    using DOT  = DesignOptimizationTerm<Object_T>;
    using DOOT = DesignOptimizationObjectiveTerm<Object_T>;
    ...
    Real getBoundaryNormalWeight() const { return m_boundaryNormalWeight; }
    void setBoundaryNormalWeight(Real w) {        m_boundaryNormalWeight = w; this->update(); }

    Real getBoundaryTangentialWeight() const { return m_boundaryTangentialWeight; }
    void setBoundaryTangentialWeight(Real w) {        m_boundaryTangentialWeight = w; this->update(); }

    Real getBoundaryTorqueWeight() const { return m_boundaryTorqueWeight; }
    void setBoundaryTorqueWeight(Real w) {        m_boundaryTorqueWeight = w; this->update(); }
    ...
    std::tuple<MatX3_T<Real_>, VecX_T<Real_>, MatX3_T<Real_>> jointForces(const Object_T<Real_> &obj) const {
        const size_t nj = obj.numJoints();
        ...
        forces          .resize(nj, 3);
        separationForces.resize(nj);
        torques         .resize(nj, 3);

        VecX_T<Real_> rf = obj.rivetForces(Object_T<Real_>::EnergyType::Elastic, true);
        for (size_t ji = 0; ji < nj; ++ji) {
            const auto &j = obj.joint(ji);
            Real s = m_separationNormalSign(j);
            Vec3_T<Real_> f = rf.template segment<3>(obj.dofOffsetForJoint(ji));
            forces.row(ji) = f;
            separationForces[ji] = s * j.normal().dot(f);
            torques.row(ji) = rf.template segment<3>(obj.dofOffsetForJoint(ji) + 3);
        }
        return result;
    }

    const JointForces &jointForces() const {
        if (!m_cachedForces) m_cachedForces = std::make_unique<JointForces>(jointForces(m_obj));
...
protected:
    ...
    Real m_normalWeight = 1.0, m_tangentialWeight = 0.0, m_torqueWeight = 0.0,
         m_boundaryNormalWeight = 1.0, m_boundaryTangentialWeight = 0.0, m_boundaryTorqueWeight = 0.0, // weights for joints of valence < 4
    ...
    template<class Joint_>
    static Real m_separationNormalSign(const Joint_ &j) {
        return (j.type == Joint_::Type::A_OVER_B) ? 1.0 : -1.0;
    }

    // (Optional) update cached quantities shared between energy/gradient calculations.
    virtual void m_update() override {
        m_cachedForces.reset();
        m_cachedForces = std::make_unique<JointForces>(jointForces());
    }

    std::tuple<VXd, VXd, VXd> getJointWeights() const {
        std::tuple<VXd, VXd, VXd> result;
        const size_t nj = m_obj.numJoints();
        std::get<0>(result).setConstant(nj,     m_normalWeight);
        std::get<1>(result).setConstant(nj, m_tangentialWeight);
        std::get<2>(result).setConstant(nj,     m_torqueWeight);
        for (size_t ji = 0; ji < nj; ++ji) {
            if (m_obj.joint(ji).valence() < 4) {
                std::get<0>(result)[ji] = m_boundaryNormalWeight;
                std::get<1>(result)[ji] = m_boundaryTangentialWeight;
                std::get<2>(result)[ji] = m_boundaryTorqueWeight;
            }
        }
        return result;
    }
    ...
    virtual Real m_value() const override {
        ...
        MX3d torques;
        std::tie(forces, separationForces, torques) = jointForces();
    ...
    VecX_T<Real_> m_grad_impl(const Object_T<Real_> &obj) const {
        ...
        std::tie(forces, separationForces, torques) = jointForces(obj);
        ...
        for (size_t ji = 0; ji < nj; ++ji) {
            const auto &j = obj.joint(ji);
            Real s = m_separationNormalSign(j);
            dJ_df.template segment<3>(obj.dofOffsetForJoint(ji)) =
                    normalWeights[ji] * std::max<Real_>(separationForces[ji] - m_epsilon, 0.0) * s * j.normal() +
                tangentialWeights[ji] * (forces.row(ji).transpose() - separationForces[ji] * s * j.normal());
            dJ_df.template segment<3>(obj.dofOffsetForJoint(ji)+3) = 
                    torqueWeights[ji] * torques.row(ji).transpose();
        }
        VecX_T<Real_> result = -obj.RodLinkage_T<Real_>::applyHessianPerSegmentRestlen(dJ_df, mask); // rivet forces are -dE/dx; explicitly use the RodLinkage energy to skip the surface attraction term.
        ...
        for (size_t ji = 0; ji < nj; ++ji) {
            const auto &j = obj.joint(ji);
            Real s = m_separationNormalSign(j);
            result.template segment<3>(obj.dofOffsetForJoint(ji) + 3) +=
                (     normalWeights[ji] * std::max<Real_>(separationForces[ji] - m_epsilon, 0.0) * s * forces.row(ji)
                - tangentialWeights[ji] *                 separationForces[ji]                   * s * forces.row(ji)) *
